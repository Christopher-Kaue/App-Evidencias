...
/** Base padrao local sem .env: servidor PHP embutido em backend/ (npm run dev:api). XAMPP: use NEXT_PUBLIC_API_BASE_URL em .env.local */
...
export function getApiBaseUrl(): string {
  if (useEdgeProxy) {
    ...
    if (process.env.VERCEL_URL) {
      return trimSlash(`https://${process.env.VERCEL_URL}`);
    }
    const port = process.env.PORT || "3000";
    return trimSlash(`http://127.0.0.1:${port}`);
  }
